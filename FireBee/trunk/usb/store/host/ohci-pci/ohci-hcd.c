...
#include "../../debug.h"
/* Extra debug information, aside from config.h */
...
struct pci_device_id usb_pci_table[] = {
	{ PCI_VENDOR_ID_AL, PCI_DEVICE_ID_AL_M5237, PCI_ANY_ID, PCI_ANY_ID, 0, 0, 0 }, /* ULI1575 PCI OHCI module ids */
	{ PCI_VENDOR_ID_NEC, PCI_DEVICE_ID_NEC_USB, PCI_ANY_ID, PCI_ANY_ID, 0, 0, 0 }, /* NEC PCI OHCI module ids */
//	{ PCI_VENDOR_ID_PHILIPS, PCI_VENDOR_ID_PHILIPS_ISP1561, PCI_ANY_ID, PCI_ANY_ID, 0, 0, 0 }, /* Philips 1561 PCI OHCI module ids */
...
#if 0
unsigned short cpu_to_le16(unsigned short val)
{
...
unsigned long cpu_to_le32(unsigned long val)
{
...
{
	DEBUG_HOST("%s URB:[%4x] dev:%2lu,ep:%2lu-%c,type:%s,len:%d/%d stat:%#lx",
			str,
	...
	if(!small)
	{
		...
		{
			DEBUG_HOST(__FILE__ ": cmd(8):");
			for(i = 0; i < 8 ; i++)
				DEBUG_HOST(" %02x", ((__u8 *)setup)[i]);
			DEBUG_HOST("\r\n");
		}
		if(transfer_len > 0 && buffer)
		{
			DEBUG_HOST(__FILE__ ": data(%d/%d):", (purb ? purb->actual_length : 0), transfer_len);
			len = usb_pipeout(pipe)? transfer_len : (purb ? purb->actual_length : 0);
			for(i = 0; i < 16 && i < len; i++)
				DEBUG_HOST(" %02x", ((__u8 *)buffer)[i]);
			DEBUG_HOST("%s\r\n", i < len? "...": "");
		}
	}
#endif
...
void ep_print_int_eds(ohci_t *ohci, char *str)
{
	...
	for(i = 0; i < 32; i++)
	{
		...
		if(*ed_p == 0)
			continue;
		DEBUG_HOST(__FILE__ ": %s branch int %2d(%2x):", str, i, i);
		while(*ed_p != 0 && j--)
		{
			ed_t *ed = (ed_t *)m32_swap((unsigned long)ed_p);
			DEBUG_HOST(" ed: %4x;", ed->hwINFO);
			ed_p = &ed->hwNextED;
		}
		DEBUG_HOST("\r\n");
	}
...
{
	DEBUG_HOST("%s: 0x%08x%s%s%s%s%s%s%s%s%s",
		label,
		mask,
...
{
	ed_t *edp; 
	value += gohci.dma_offset;
	edp = (ed_t *)value;
	if(value && (value < 0xDFFFF0))  /* STRAM */
	{
		DEBUG_HOST("%s %08x", label, value);
		DEBUG_HOST("%08x", edp->hwINFO);
		DEBUG_HOST("%08x", edp->hwTailP);
		DEBUG_HOST("%08x", edp->hwHeadP);
		DEBUG_HOST("%08x", edp->hwNextED);
	}
}
...
static void ohci_dump_status(ohci_t *controller)
{
	...
	if(temp != 0x10)
		DEBUG_HOST("spec %d.%d", (temp >> 4), (temp & 0x0f));
	temp = readl(&regs->control);
	DEBUG_HOST("control: 0x%08x%s%s%s HCFS=%s%s%s%s%s CBSR=%d", temp,
	...
	DEBUG_HOST("cmdstatus: 0x%08x SOC=%d%s%s%s%s", temp,
		(temp & OHCI_SOC) >> 16,
		(temp & OHCI_OCR) ? " OCR" : "",
		(temp & OHCI_BLF) ? " BLF" : "",
		(temp & OHCI_CLF) ? " CLF" : "",
...
static void ohci_dump_roothub(ohci_t *controller, int verbose)
{
	...
	if(verbose)
	{
		DEBUG_HOST("roothub.a: %08x POTPGT=%d%s%s%s%s%s NDP=%d", temp,
		...
		DEBUG_HOST("roothub.b: %08x PPCM=%04x DR=%04x",
			temp,
			(temp & RH_B_PPCM) >> 16,
			(temp & RH_B_DR)
			);
		temp = roothub_status(controller);
		DEBUG_HOST("roothub.status: %08x%s%s%s%s%s%s",
			temp,
	...
	for(i = 0; i < ndp; i++)
	{
		...
		DEBUG_HOST("roothub.portstatus [%d] = 0x%08x%s%s%s%s%s%s%s%s%s%s%s%s",
			i,
			temp,
...
{
	DEBUG_HOST("OHCI controller usb-%s state", controller->slot_name);
	/* dumps some of the state we know about */
	ohci_dump_status(controller);
	if(verbose)
		ep_print_int_eds(controller, "hcca");
	DEBUG_HOST("hcca frame #%04x", controller->hcca->frame_no);
...
static inline int sohci_return_job(struct ohci *hc, urb_priv_t *urb)
{
	...
	switch(usb_pipetype(urb->pipe))
	{
		case PIPE_INTERRUPT:
			/* implicitly requeued */
			if(urb->dev->irq_handle && (urb->dev->irq_act_len = urb->actual_length))
			{
				writel(OHCI_INTR_WDH, &regs->intrenable);
				readl(&regs->intrenable); /* PCI posting flush */
 				/* call interrupt device routine */
 				DEBUG_HOST("irq_handle device %d", urb->dev->devnum);
				urb->dev->irq_handle(urb->dev);
				writel(OHCI_INTR_WDH, &regs->intrdisable);
				readl(&regs->intrdisable); /* PCI posting flush */
			}
			urb->actual_length = 0;
			td_submit_job(urb->dev,	urb->pipe, urb->transfer_buffer, urb->transfer_buffer_length, NULL, urb, urb->interval);
			break;
		case PIPE_CONTROL:
		case PIPE_BULK:
			break;
...
static void periodic_unlink(struct ohci *ohci, volatile struct ed *ed,
			    unsigned idx, unsigned period)
{
	for( ;idx < NUM_INTS; idx += period)
	{
		__u32	*ed_p = &ohci->hcca->int_table[idx];
 ...
 * change
 */
static ed_t *ep_add_ed(struct usb_device *usb_dev, unsigned long pipe, int interval, int load)
{
	td_t *td;
	ed_t *ed_ret;
	volatile ed_t *ed;
	struct ohci_device *ohci_dev = gohci.ohci_dev;
	...
	if(ed->state == ED_NEW)
	{
		...
		td = td_alloc(usb_dev);
		ed->hwTailP = m32_swap((unsigned long)td - gohci.dma_offset);
...
static void td_fill(ohci_t *ohci, unsigned int info, void *data, int len,
 struct usb_device *dev, int idx, urb_priv_t *urb_priv)
{
	volatile td_t *td, *td_pt;
#ifdef OHCI_FILL_TRACE
	int i;
#endif
	if(idx > urb_priv->length)
	{
		err("index > length");
		return;
	}
	/* use this td as the next dummy */
	td_pt = urb_priv->td[idx];
	td_pt->hwNextTD = 0;
	/* fill the old dummy TD */
	td = urb_priv->td[idx] = (td_t *)((m32_swap(urb_priv->ed->hwTailP) & ~0xf) + ohci->dma_offset);
	td->ed = urb_priv->ed;
	td->next_dl_td = NULL;
	td->index = idx;
	td->data = (__u32)data;
 ...
 void *buffer, int transfer_len, struct devrequest *setup, urb_priv_t *urb, int interval)
{
	...
	if(urb->length != cnt)
		DEBUG_HOST("TD LENGTH %d != CNT %d", urb->length, cnt);
}
...
static void dl_transfer_length(td_t *td)
{
	...
	if(tdBE)
		tdBE += gohci.dma_offset;
	if(tdCBP)
		tdCBP += gohci.dma_offset;
...
/*-------------------------------------------------------------------------*/
void check_status(td_t *td_list)
{
...
static void finish_urb(ohci_t *ohci, urb_priv_t *urb, int status)
{
	if((status & (ED_OPER | ED_UNLINK)) && (urb->state != URB_DEL))
		...
		DEBUG_HOST("finish_urb: strange.., ED state %x, \r\n", status);
}
 ...
 */
int takeback_td(ohci_t *ohci, td_t *td_list)
{
	ed_t *ed;
	int cc;
	int stat = 0;
	...
	* and mark the URB finished if so */
	...
	DEBUG_HOST("dl_done_list: processing TD %x, len %x\r\n",
	...
	switch(bmRType_bReq)
	{
		...
		*/
		...
		case RH_GET_DESCRIPTOR:
			switch((wValue & 0xff00) >> 8)
			{
				case(0x01): /* device descriptor */
					len = min_t(unsigned int, leni, min2_t(unsigned int, sizeof(root_hub_dev_des), wLength));
					data_buf = root_hub_dev_des;
					OK(len);
				case(0x02): /* configuration descriptor */
					len = min_t(unsigned int, leni, min2_t(unsigned int, sizeof(root_hub_config_des), wLength));
					data_buf = root_hub_config_des;
					OK(len);
				case(0x03): /* string descriptors */
					if(wValue == 0x0300)
					{
						len = min_t(unsigned int, leni, min2_t(unsigned int, sizeof(root_hub_str_index0), wLength));
						data_buf = root_hub_str_index0;
						OK(len);
					}
					if(wValue == 0x0301)
					{
						len = min_t(unsigned int, leni, min2_t(unsigned int, sizeof(root_hub_str_index1), wLength));
		...
		case RH_GET_DESCRIPTOR | RH_CLASS:
		{
			...
			len = min_t(unsigned int, leni, min2_t(unsigned int, data_buf [0], wLength));
			OK(len);
		}
		case RH_GET_CONFIGURATION:	*(__u8 *) data_buf = 0x01; OK(1);
		case RH_SET_CONFIGURATION:	WR_RH_STAT(0x10000); OK(0);
		default:
			DEBUG_HOST("unsupported root hub command");
			stat = USB_ST_STALLED;
	}
#if DEBUG_HOST_LAYER
		...
		int transfer_len, struct devrequest *setup, int interval)
{
	...
	int timeout;
	urb_priv_t *urb;
	urb = (urb_priv_t *)usb_malloc(sizeof(urb_priv_t));
	if(urb == NULL)
	{
...
#define BULK_TO	 5000	/* timeout in milliseconds */
	...
	while(1)
	{
		...
		{
			err("CTL:TIMEOUT ");
			DEBUG_HOST("submit_common_msg: TO status %x\r\n", stat);
			urb->finished = 1;
			stat = USB_ST_CRC_ERR;
			break;
		}
...
//	wait_ms(1);
	...
	if(((pipe >> 8) & 0x7f) == gohci.rh.devnum)
	{
		gohci.rh.dev = dev;
		/* root hub - redirect */
		return ohci_submit_rh_msg(dev, pipe, buffer, transfer_len, setup);
	}
...
{
	info("submit_int_msg");
	return submit_common_msg(dev, pipe, buffer, transfer_len, NULL, interval);
}
...
static int hc_reset(ohci_t *ohci)
{
	...
	DEBUG_HOST("USB HC reset_hc usb-%s: ctrl = 0x%X ;\r\n", ohci->slot_name, readl(&ohci->regs->control));
...
static int hc_interrupt(void)
{
	...
	else
	{
		...
		else
		{
			...
			if(ints == 0)
			{
//				DEBUG_HOST("hc_interrupt: returning..\r\n");
				return 0xff;
			}
		}
	}
  DEBUG_HOST("Interrupt: 0x%x frame: 0x%x", ints, le16_to_cpu(ohci->hcca->frame_no));
...
//		wait_ms(1);
	...
	{
		DEBUG_HOST("USB Schedule overrun\r\n");
...
static int handle_usb_interrupt(void)
{
#ifdef COLDFIRE /* no bus snooping on Coldfire */
	extern void flush_caches(void);
	conout_debug('.');
	flush_caches(); /* native interrupt */
#endif /* COLDFIRE */
	gohci.irq = 0;
	hc_interrupt();
	gohci.irq = -1;
...
int usb_lowlevel_init(long handle, const struct pci_device_id *ent)
{
	...
	else if(!gohci.handle) /* for restart USB cmd */
		return(-1);	
	/* this must be aligned to a 256 byte boundary */
	gohci.hcca_unaligned = (struct ohci_hcca *)usb_malloc(sizeof(struct ohci_hcca) + 256);
	if(gohci.hcca_unaligned == NULL)
	{
		err("HCCA malloc failed");
		return -1;
	}
	/* align the storage */
	gohci.hcca = (struct ohci_hcca *)(((unsigned long)gohci.hcca_unaligned + 255) & ~255);
	memset(gohci.hcca, 0, sizeof(struct ohci_hcca));
	info("aligned ghcca 0x%p", gohci.hcca);
	gohci.ohci_dev_unaligned = (struct ohci_device *)usb_malloc(sizeof(struct ohci_device) + 8);
	if(gohci.ohci_dev_unaligned == NULL)
	{
		err("EDs malloc failed");
		hc_free_buffers(&gohci);
		return -1;
	}
	gohci.ohci_dev = (struct ohci_device *)(((unsigned long)gohci.ohci_dev_unaligned + 7) & ~7);
	memset(gohci.ohci_dev, 0, sizeof(struct ohci_device));
	info("aligned EDs 0x%p", gohci.ohci_dev);
	gohci.td_unaligned = (td_t *)usb_malloc(sizeof(td_t) * (NUM_TD + 1));
	if(gohci.td_unaligned == NULL)
	{
		err("TDs malloc failed");
		hc_free_buffers(&gohci);
		return -1;
	}
	ptd = (td_t *)(((unsigned long)gohci.td_unaligned + 7) & ~7);
	...
	if((long)pci_rsc_desc >= 0)
	{
		...
		do
		{
			DEBUG_HOST("PCI USB descriptors: flags 0x%08x start 0x%08x \r\n offset 0x%08x dmaoffset 0x%08x length 0x%08x",
			...
			if(!(pci_rsc_desc->flags & FLG_IO))
			{
				if(usb_base_addr == 0xFFFFFFFF)
				{
					...
					gohci.regs = (void *)(pci_rsc_desc->offset + pci_rsc_desc->start);
					gohci.dma_offset = pci_rsc_desc->dmaoffset;
					if((pci_rsc_desc->flags & FLG_ENDMASK) == ORD_MOTOROLA)
						gohci.big_endian = 0; /* host bridge make swapping intel -> motorola */
					else
						gohci.big_endian = 1; /* driver must swapping intel -> motorola */
				}
			}
			flags = pci_rsc_desc->flags;
			pci_rsc_desc += (unsigned long)pci_rsc_desc->next;
		}
	...
	if(handle && (ent != NULL))
	{
		switch(ent->vendor)
		{
			case PCI_VENDOR_ID_AL: gohci.slot_name = "uli1575"; break;
			case PCI_VENDOR_ID_NEC: gohci.slot_name = "uPD720101"; break;
			case PCI_VENDOR_ID_PHILIPS: gohci.slot_name = "isp1561"; break;
			default: gohci.slot_name = "generic"; break;
		}
	}
	(void) Cconws("OHCI usb-");
	(void) Cconws(gohci.slot_name);
	(void) Cconws(", regs address 0x");
	ltoa(buf, (long)gohci.regs, 16);
	(void) Cconws(buf);
	(void) Cconws(", PCI handle 0x");
	ltoa(buf, handle, 16);
	(void) Cconws(buf);
	(void) Cconws("\r\n");
	gohci.flags = 0;
...
#ifndef CONFIG_USB_INTERRUPT_POLLING
#ifdef PCI_XBIOS
	hook_interrupt(handle, handle_usb_interrupt, NULL);
#else
	Hook_interrupt(handle, (void *)handle_usb_interrupt, NULL);
#endif /* PCI_BIOS */
#endif /* CONFIG_USB_INTERRUPT_POLLING */
	ohci_inited = 1;
	return 0;
