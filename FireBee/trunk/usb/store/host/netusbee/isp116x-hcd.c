...
/* For NetUSBee, take the raw_read out from read functions, we want to swap the bytes
   to read correct values because NetUSBee swaped the bytes by hardware before we read 
   them */
...
//		return -1;
	...
	if (usb_pipebulk(pipe))
		...
		timeout = 1000; /* Galvez : netusbee : default = 100 */
...
static int GALVEZ_test_function( struct isp116x *isp116x )
{
	...
	isp116x_write_reg16(isp116x, HCSCRATCH, 0x1234);
	res = isp116x_read_reg16(isp116x, HCSCRATCH);	
	ERR ("Scratch register read: %x\r\n",res);
	return 0;
...
//	u16 val;
	...
	isp116x->data_reg = (u16 *) ISP116X_HCD_DATA_BEE;
...
//	GALVEZ_test_function ( isp116x );	/* Gálvez: Testing writing to registers */
